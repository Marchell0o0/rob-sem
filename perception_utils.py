...
def viz_calibration_boards(
    corners3d, grid, rvecs, tvecs, *, axis_length=0.1, text_size=12
):
    """Visualize multiple chessboard patterns with one camera.

    Args:
        corners3d: Original 3D points of one chessboard pattern
        grid: Tuple of (height, width) of the chessboard
        rvecs: List of rotation vectors for each board position
        tvecs: List of translation vectors for each board position
    ...
    # Transform and plot each chessboard pattern
    for i, (rvec, tvec) in enumerate(zip(rvecs, tvecs)):
        ...
        # Draw board as a solid rectangle
        verts = [corners]
        rect = Poly3DCollection(verts, alpha=0.3)
        rect.set_facecolor('gray')
        rect.set_edgecolor('black')
        ax.add_collection3d(rect)

        # Calculate board center for axes
        ...
        ax.text(center[0], center[1], center[2],
                f'Board {i}', size=text_size, color='k')

    # Plot camera at origin
    ax.scatter(0, 0, 0, c='black', marker='o', s=100, label='Camera')
